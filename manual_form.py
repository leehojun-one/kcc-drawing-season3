...
def _build_extract_prompt():
    ...
    return (
        "당신은 한국 KCC홈씨씨 창호 '수기 실측지'를 읽어 표 데이터로 변환하는 전문가입니다.\n"
        "이미지의 '3. 실측 내역' 표에서 **모든 행을 빠짐없이** 읽어 JSON 배열로만 출력하세요.\n"
        "설명/마크다운/코드 블록 금지. 순수 JSON만 출력.\n"
        "표가 18행이면 18개, 15행이면 15개 — **끝까지 전부** 읽으세요. 절대 중간에 멈추지 마세요.\n\n"

        "★★★ 핵심 규칙: 통바 vs 공틀 구분 ★★★\n"
        "실측지에서 '통바'라고 적힌 행이 두 종류입니다:\n"
        "  A) '사면공통' / '사면통바' 행 (가로W × 세로H 치수가 있음)\n"
        "     → 이것은 **공틀창**(별도 도면 객체)입니다.\n"
        "     → 제품='공틀창225', 형태='3W(1:2:1)'(또는 적힌 형태), 가로W/세로H를 그대로 읽으세요.\n"
        "     → **바로 위 창**의 '사면통바' 칸에도 해당 통바종류(CB-100*45 등)를 넣어주세요.\n"
        "  B) 표 하단 '4. 기타 특이사항' 비고란에 적힌 통바 (길이 L만 있음, 높이 없음)\n"
        "     → 이것은 진짜 통바입니다. 이건 읽지 않아도 됩니다(별도 처리).\n\n"

        "각 객체 키와 규칙:\n"
        "- 위치: 적용공간 텍스트 (예: 거실발코니, 안방확장, 세탁좌F)\n"
        f"- 제품: 다음 중 하나 → {PRODUCT_LIST}\n"
        "  (약어 매핑: 141단창/발코니141→발코니창141, 251이중/발코니251→발코니이중창251, "
        "230이중→일반이중창230, 115단창→일반단창115, 225공틀/225공통→공틀창225, "
        "100*45공통/사면공통→공틀창225, 고정창/Fix/F→고정창, 미는문/터닝→터닝도어)\n"
        f"- 형태: 다음 중 하나 → {SHAPE_LIST}\n"
        "  (약어: 3W=3W(1:2:1), 2W=2W, U/B=2W(U/B), F/Fix/고정=고정창형태)\n"
        "- 가로W: 정수(mm). 표의 '가로' 칸\n"
        "- 세로H: 정수(mm). 표의 '세로' 칸\n"
        "- 벤트W1: 정수(mm). 특이사항에 'VENT 1100' 또는 벤트 치수가 있으면 그 값. 없으면 0\n"
        f"- 방향: 다음 중 하나 → {VENT_DIRS} (좌/우/좌힌지/우힌지 → 좌 또는 우. 명시 없으면 N)\n"
        f"- 내부유리: 다음 중 하나 → {GLASS_INNER}\n"
        "  (약어: 더블로이/DL→P_28T 더블로이+투명(V), 투명로이→P_24T 투명+로이(V), "
        "미스트로이→P_24T 미스트+로이(V), 3로이→P_24T 더블로이+투명(V))\n"
        f"- 외부유리: 이중창만, 다음 중 하나 → {GLASS_OUTER} (단창이면 \"(없음)\")\n"
        "- 핸들높이: 정수(mm). 특이사항에 '핸들 450' 등 있으면 그 값. 없으면 0\n"
        "- 방충망: 특이사항에 '망' 또는 '#' 표기 있으면 true, 없으면 false\n"
        f"- 사면통바: 다음 중 하나 → {TONGBA_4SIDE}.\n"
        "  해당 창에 사면통바가 필요하면 CB-100*45 등을 넣고, 아니면 \"(없음)\".\n"
        "  위 A)항의 공틀 행이 바로 아래에 있다면 이 창의 사면통바에도 같은 값.\n\n"

        "★ 표 끝까지 전부 읽으세요. 행 수 제한 없습니다.\n"
        "★ 확실치 않은 칸은 빈값/0/기본값으로 두되, 가로·세로·위치 숫자는 최대한 정확히.\n"
        "★★★ 반드시 한 줄 압축 JSON으로 출력하세요! 들여쓰기/줄바꿈 절대 금지! "
        "토큰을 아끼기 위해 가장 짧은 형태로 써야 전체 표를 다 출력할 수 있습니다.\n\n"
        "출력 형식 (한 줄, 쉼표로 이어붙임):\n"
        "[{\"위치\":\"거실\",\"제품\":\"발코니창141\",\"형태\":\"3W(1:2:1)\",\"가로W\":4635,"
        "\"세로H\":2330,\"벤트W1\":0,\"방향\":\"N\",\"내부유리\":\"P_28T 더블로이+투명(V)\","
        "\"외부유리\":\"(없음)\",\"핸들높이\":0,\"방충망\":true,\"사면통바\":\"(없음)\"},"
        "{\"위치\":\"안방\",\"제품\":\"발코니이중창251\",\"형태\":\"3W(1:2:1)\",\"가로W\":5930,"
        "\"세로H\":2490,\"벤트W1\":0,\"방향\":\"N\",\"내부유리\":\"P_28T 더블로이+투명(V)\","
        "\"외부유리\":\"28T 투명+투명(V)\",\"핸들높이\":0,\"방충망\":true,\"사면통바\":\"CB-100*45\"}]"
    )
